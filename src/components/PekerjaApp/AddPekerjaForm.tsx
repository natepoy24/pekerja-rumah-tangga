...
function SubmitButton() {
  ...
  return (
    ...
    >
      ...
      <span>{pending ? "Menyimpan Profil..." : "Terbitkan Profil Pekerja"}</span>
    </button>
  );
}
...
export default function AddPekerjaForm() {
  ...
  const [state, formAction] = useActionState(addPekerja, initialState as any);
  ...
  const [hasBahasaLainnya, setHasBahasaLainnya] = useState(false);
  const [bahasaLainnyaText, setBahasaLainnyaText] = useState("");

  // Masakan khusus state
  const [hasMasakanKhusus, setHasMasakanKhusus] = useState(false);
  const [masakanKhususText, setMasakanKhususText] = useState("");
  ...
  useEffect(() => {
    if (state?.success === true) {
      setModalSuccessMessage(state.message || "Kandidat pekerja berhasil ditambahkan ke katalog!");
  ...
  return (
    <div className="max-w-4xl mx-auto font-sans relative">
      ...
      <div className="mb-8">
        ...
        <h2 className="font-serif text-3xl font-bold text-[#14201D] tracking-tight">Tambah Kandidat Pekerja Baru</h2>
        <p className="text-sm text-on-surface-variant mt-1">Lengkapi informasi profil pekerja untuk dipublikasikan ke katalog.</p>
      </div>
      ...
      <form action={handleFormSubmit} className="space-y-8">
        ...
        {/* SECTION 2: KATEGORI & KETERAMPILAN MULTI-SELECT */}
        <section className="bg-white p-6 rounded-2xl border border-[#D5E8D0] shadow-sm space-y-6">
          <div className="flex items-center gap-3 border-b border-outline-variant/20 pb-4">
        ...
        <section className="bg-white p-6 rounded-2xl border border-[#D5E8D0] shadow-sm space-y-6">
          <div className="flex items-center gap-3 border-b border-outline-variant/20 pb-4">
            <h3 className="font-serif text-xl font-bold text-[#14201D]">4. Spesifikasi Fisik & Catatan Khusus</h3>
          </div>
          ...
          <div className="space-y-3 pt-2">
            <label className="flex items-center gap-3 text-sm font-medium text-on-surface cursor-pointer">
              <input type="checkbox" name="bisa_bawa_motor" className="rounded text-[#0B4F42] focus:ring-[#0B4F42]" />
              <span>Bisa Mengendarai Sepeda Motor</span>
            </label>

            <label className="flex items-center gap-3 text-sm font-medium text-on-surface cursor-pointer">
              <input type="checkbox" name="takut_anjing" className="rounded text-[#0B4F42] focus:ring-[#0B4F42]" />
              <span>Takut Anjing / Hewan Peliharaan Berbulu</span>
            </label>

            <label className="flex items-center gap-3 text-sm font-medium text-on-surface cursor-pointer">
              <input type="checkbox" name="bisa_masak_babi" className="rounded text-[#0B4F42] focus:ring-[#0B4F42]" />
              <span>Bisa Memasak / Mengolah Daging Babi</span>
            </label>
          </div>

          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-1">
              Ringkasan Deskripsi Profil / Catatan Tambahan
            </label>
            <textarea
              name="deskripsi"
              rows={4}
              ...
              placeholder="Tuliskan kepribadian, latar belakang pengasingan, atau catatan khusus calon pekerja..."
            ></textarea>
          </div>
        </section>
        ...
        <section className="bg-white p-6 rounded-2xl border border-[#D5E8D0] shadow-sm space-y-6">
          <div className="flex items-center gap-3 border-b border-outline-variant/20 pb-4">
            ...
            <h3 className="font-serif text-xl font-bold text-[#14201D]">5. Unggah Foto Profil Kandidat</h3>
          </div>
          ...
          <div>
            <input
              ...
              className="w-full text-sm text-on-surface file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-[#EBF4E7] file:text-[#3E7B28] hover:file:bg-[#D5E8D0] transition-colors cursor-pointer"
            />
            <p className="text-xs text-on-surface-variant/70 mt-2">
              Format yang didukung: JPG, PNG, WEBP. Maksimal 3MB. Foto akan disimpan ke bucket Supabase storage <code className="bg-[#EBF4E7] px-1 rounded font-semibold text-[#0B4F42]">foto-pekerja</code>.
            </p>
          </div>
